...
if __name__ == '__main__':
  ...
  with tf.Session() as sess:
    ...
    reshaped_image = inputs_origin('/tmp/cifar10_data/cifar-10-batches-bin')
    ...
    if not os.path.exists('/tmp/cifar10_data/raw/'):
      os.makedirs('/tmp/cifar10_data/raw/')
    # 保存30张图片
    for i in range(30):
      # 每次sess.run(reshaped_image)，都会取出一张图片
      image_array = sess.run(reshaped_image)
      # 将图片保存
      scipy.misc.toimage(image_array).save('/tmp/cifar10_data/raw/%d.jpg' % i)